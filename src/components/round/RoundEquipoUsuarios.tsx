      ...
      >
        ...
        <span className="flex-1 min-w-0">
          <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground block">
            Añadir usuarios <span className="font-normal normal-case">(opcional)</span>
          </span>
          {!open && countSel > 0 && (
      ...
      {open && (
        <div className="px-3 pb-3 pt-0 border-t border-border/60 space-y-2">
          <p className="text-[10px] text-muted-foreground leading-snug">
            Usuarios con la misma asignación ({region} · {distrito}
            {servicio ? ` · ${servicio}` : ''}). Aparecen en el informe si los marcás.
          </p>
          {loading && (
          ...
          <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
            {miembros.map((m) => {
              const on = seleccionados.includes(m.display_name);
              return (
                <button
                  key={m.user_id}
                  type="button"
                  onClick={() => onToggle(m.display_name)}
